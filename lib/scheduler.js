...
const agenda = new Agenda({db: {address: MONGO_HOST,  collection: 'scheduler',processEvery: '3 seconds'}},);
...
function Scheduler(bot) {
    ...
    */
    this.setReminder = async function (channel, args) {
        ...
        await channel.send(`OK, **${date}** I will remind **${message}**`);
    }
    ...
    this.deleteReminder = async function (channel, args) {
        ...
        try {
            ...
            if (jobs.length === 0) {
                await channel.send(`You have no anal reminder set with this name`);
                return;
            }
            else {
                await Promise.all(jobs.map(j=> j.remove()))
                 await channel.send(`Anal reminder ${analName} removed`);
            }
        } catch (error) {
            log(error)
        }
